...
public class UCClient extends PHPFunctions {
	...
	public String uc_authcode(String string, String operation, String key,
		...
		String cryptkey = keya
				+ md5(new StringBuilder(String.valueOf(keya)).append(keyc)
						.toString());
		int key_length = cryptkey.length();

		string = sprintf("%010d", expiry > 0 ? expiry + time() : 0L)
				+ substr(
						md5(new StringBuilder(String.valueOf(string)).append(
								keyb).toString()), 0, 16) + string;
		...
		if (operation.equals("DECODE")) {
			...
			if ((CommUtil.null2Int(substr(result.toString(), 0, 10)) == 0)
					|| (CommUtil.null2Long(substr(result.toString(), 0, 10))
							.longValue() - time() > 0L)) {
				if (substr(result.toString(), 10, 16)
						.equals(substr(
								md5(substr(result.toString(), 26) + keyb), 0,
								16)))
